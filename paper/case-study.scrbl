...
for over 25% of the code of each category. Most of the arithemtic
proofs that do not involve Log are dispatched by the standard Coq
tactic @tt{omega}.
...
the value would be negligible because each of the intermediate
functions requires so insight to produce and requires far more effort
that the initial version.

In both cases---single step precise statements and progressively
abstract statements---the verifier needs to have an intution for what
